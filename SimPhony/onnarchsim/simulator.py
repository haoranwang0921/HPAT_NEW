...
class ONNArchSimulator(object):
    ...
    def initialize(self) -> None:
        ...
        if (
            ...
            with open(self.params.onn_conversion_cfg, "r", encoding="utf-8") as file:
            ...
            try:
                with open(self.params.nn_conversion_cfg, "r", encoding="utf-8") as file:
                    map_cfgs = yaml.safe_load(file)
        ...
        with open(self.params.model2arch_map_cfg, "r", encoding="utf-8") as file:
    ...
    def simu_latency(
        ...
        for sub_arch_name, memory_latency in sub_arch_memory_latency.items():
            comp_lat = sub_arch_computation_latency[sub_arch_name]
            # Handle new structured latency dict (B4)
            if isinstance(comp_lat, dict):
                comp_total = (
                    comp_lat.get("compute_latency_s", 0.0)
                    + comp_lat.get("operand_encoding_latency_s", 0.0)
                    + comp_lat.get("conversion_latency_s", 0.0)
                    + comp_lat.get("programming_latency_s", 0.0)
                )
                # Fallback to legacy keys if new fields are zero
                if comp_total == 0.0:
                    comp_total = (
                        comp_lat.get("total_tuning_latency", 0.0)
                        + comp_lat.get("total_static_latency", 0.0)
                    )
                sub_arch_total_latency[sub_arch_name] = memory_latency + comp_total
            else:
                sub_arch_total_latency[sub_arch_name] = memory_latency + comp_lat
