...
def extract_layer_info(model):
    ...
    for name, layer in model.named_modules():
        ...
        elif QMatMul is not None and isinstance(layer, QMatMul):  # noqa: F405
            layer_info[name] = (
                "MatMul",
                layer,
                (0, 0, 0, 0),
        ...
        elif "MatMul" in layer_type or "matmul" in layer_type.lower():
            layer_info[name] = (
                "MatMul",
                layer,
                (0, 0, 0, 0),
...
def im2col_2d_shape(
    ...
    if layer == "Linear":
        ...
        K = input_size_1[-1]
        if len(input_size_1) == 2:
            M = input_size_1[0]
            N = input_size_2[0] if len(input_size_2) == 2 else math.prod(input_size_2[:-1])
        else:
            # Batched matmul: [..., M, K] @ [..., K, N]
            # Matrix dims are the last 2; leading dims are independent batch iterations
            batch = math.prod(input_size_1[:-2])
            M = input_size_1[-2] * batch
            N = input_size_2[-1]
        reshaped_matrix_size_X = (K, M)
        reshaped_matrix_size_W = (N, K)
        return reshaped_matrix_size_W, reshaped_matrix_size_X, None
