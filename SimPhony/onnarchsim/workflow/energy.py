...
def initalize_all_devices_power(
    ...
    for device, device_cfg in components.items():
        ...
        if "adc" in type:
            ...
            device_cfg["dynamic_power"] = dynamic_power
        elif "mrr" in type and "rerouter" not in type:
            cfgs = device_cfg.get("cfgs", {})
            tuning_cycles = cfgs.get("response_time", 1000.0) * work_freq
            device_cfg["tuning_cycles"] = tuning_cycles

            lam0 = 1550.0
            q_factor = cfgs.get("Q_factor", 10000)
            hwhm = lam0 / (2.0 * q_factor)
            tun_eff = cfgs.get("tuning_efficiency", 0.4167)
            dynamic_power = hwhm / tun_eff if tun_eff > 0 else 0.0
            device_cfg["dynamic_power"] = dynamic_power

            current_er = (
                cfgs["extinction_ratio"]
                if cfgs.get("extinction_ratio") is not None
                else 0
            )
            extinction_ratio = (
                current_er if current_er > extinction_ratio else extinction_ratio
            )
...
def reuse_determinator(
    ...
    """Compute per-MVM inner-loop cost, per-tile outer-loop cost, and reuse factors.

    Returns:
        operand_2_reuse_factor, operand_1_reuse_factor, max_cycle,
        inner_loop, outer_loop
    """
    if dataflow == "weight_stationary":
        # Weights (operand_1) stay resident; inputs (operand_2) change every MVM.
        inner_loop = max_operand_2_tuning_cycles
        outer_loop = max_operand_1_tuning_cycles
        operand_1_reuse = 1
        ...
        # Inputs (operand_2) stay resident; weights (operand_1) change every MVM.
        inner_loop = max_operand_1_tuning_cycles
        outer_loop = max_operand_2_tuning_cycles
        ...
        # Both operands change; no stationary reuse.
        inner_loop = max_operand_2_tuning_cycles
        outer_loop = max_operand_1_tuning_cycles
        ...
        max_cycle = ((iter_M * iter_N * iter_D) * (inner_loop + outer_loop))
    ...
    return 1 / operand_2_reuse, 1 / operand_1_reuse, max_cycle, inner_loop, outer_loop
...
def energy_calculator(
    ...
    total_computation_latency = {
        "total_tuning_latency": 0,
        "total_static_latency": 0,
        "compute_latency_s": 0.0,
        "operand_encoding_latency_s": 0.0,
    ...
    # ADC tuning cycles for conversion latency
    adc_tuning = 1
    for device in devices:
        if "adc" in device.lower():
            adc_tuning = max(adc_tuning, devices[device].get("tuning_cycles", 1))
    ...
    for layer_name, (
        ...
        (
            operand_2_duty_cycles,
            operand_1_duty_cycles,
            max_cycle,
            inner_loop,
            outer_loop,
        ) = reuse_determinator(
        ...
        # Structured latency breakdown
        # compute_latency: optical pass-through (1 cycle per MVM)
        total_computation_latency["compute_latency_s"] += (
            switching_cycles / work_freq * 1e-9
        )
        # operand_encoding_latency: per-MVM input encoding (inner_loop)
        total_computation_latency["operand_encoding_latency_s"] += (
            switching_cycles * inner_loop / work_freq * 1e-9
        )
        # conversion_latency: ADC digitization
        total_computation_latency["conversion_latency_s"] += (
            switching_cycles * adc_tuning / work_freq * 1e-9
        )
        # programming_latency: MRR tuning at tile boundaries (outer_loop)
        total_computation_latency["programming_latency_s"] += (
            iter_N * iter_D * outer_loop / work_freq * 1e-9
        )
    ...
    total_computation_latency["total_tuning_latency"] = (
        total_computation_latency["total_tuning_latency"] / work_freq * 1e-9
    )
    total_computation_latency["total_static_latency"] = (
        total_computation_latency["total_static_latency"] / work_freq * 1e-9
    )
