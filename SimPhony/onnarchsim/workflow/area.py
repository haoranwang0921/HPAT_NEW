...
def parsing_config_from_instance(
    ...
    # Strip replication suffix (_N) from graph node names like i0_0 -> i0
    import re as _re
    base_instance = _re.sub(r"_\d+$", "", provided_instance)
    for device, device_cfg in node_config.items():
        _, _, _, instance, _ = parse_device_name(device)
        if base_instance == instance or provided_instance == instance:
            # print(device_cfg)
            return device_cfg
...
def cal_node_params(
    ...
    # WDM model: allow per-node area override via config
    count_override = sub_arch["core"]["node"].get("node_count", None)
    ...
    if count_override is not None:
        count = evaluate_factors(count_override, R, C, core_height, core_width, 1)
