...
def load_required_devices(
    ...
    # Capture parent chip_type from the core-level node instance so that
    # internal devices (MRR, PD, etc.) inherit it when they have no
    # explicit chip_mapping of their own.  (Plan B2)
    parent_node_chip = (
        components.get("node", {}).get("chip_type", "off_chip")
    ...
    # Inherit parent node chip_type for devices that have no explicit mapping
    if parent_node_chip and parent_node_chip != "off_chip":
        for dev_name, dev_info in temp_components.items():
            if dev_info.get("chip_type", "off_chip") == "off_chip":
                dev_info["chip_type"] = parent_node_chip
